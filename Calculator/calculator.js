...
(function (){
   ...
   app.controller ('calculatorCtrl', ['$scope',function ($scope){
       ...
       $scope.cfl_conversion = .0416;
       $scope.led_conversion = .0125;

       $scope.calculate = function (){
          $scope.inc_wattage = ($scope.current_lumens * $scope.inc_conversion).toFixed(1);
          $scope.hal_wattage = ($scope.current_lumens * $scope.hal_conversion).toFixed();
          $scope.cfl_wattage = ($scope.current_lumens * $scope.cfl_conversion).toFixed();
          $scope.led_wattage = ($scope.current_lumens * $scope.led_conversion).toFixed();
          if ( $scope.current_hours > 24 || $scope.current_hours < 0)
          {
